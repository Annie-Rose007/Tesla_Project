---
title: "Tesla: Le prix à payer pour l'innovation ?"
...
Pour mener cette étude, nous nous appuierons sur une compilation de données[^1] accessible via le site web https://www.tesladeaths.com/. Cette base de données exhaustive inclut diverses sections, dont celle dédiée aux "Deaths" (Décès), qui comporte 23 variables pertinentes. Dans le cadre de cette analyse, nous nous concentrerons sur les variables suivantes du fichier csv:
...
* Deaths (nombre de décès causé par l'accident)
...
#Corriger l'erreur dans Year pour le "case" 431.1
...
# Graphique "Décès liés à l'Autopilot par catégorie de victime et modèle de voiture"
...
Le tablaux à droite montre le résultat de la régression linéaire du graphique "Décès liés à l'Autopilot par catégorie de victime et modèle de voiture". Commençon par expliquer chaque facteurs interessants.

     1. La P-value globale (Le modèle est-il utile ?) 

Regardez la toute dernière ligne : p-value: 6.798e-06. 

•	Elle est très inférieure à 0,05. Cela signifie que le modèle est statistiquement significatif : les variables choisies (Year, Modèle, Autopilot) ont un impact réel sur le nombre de décès, ce n'est pas dû au hasard. 

     2. Interprétation des Coefficients (L'impact de chaque variable) 

•	Year (0.038790) : La p-value est de 0,00897 (significatif). Cela indique une tendance à la hausse : chaque année supplémentaire est associée à une légère augmentation du nombre de décès (environ +0,038 par accident répertorié).

•	Model CYB (0.672110) : La p-value est de 0,02638 (significatif). Le Cybertruck semble associé à un nombre de décès par accident plus élevé (+0,67) par rapport au modèle de référence (probablement le Model 3, qui n'apparaît pas car il sert de base).

•	Model X et Model Y : Leurs p-values sont très élevées (0,89 et 0,97). Cela signifie que pour ces modèles, il n'y a pas de différence significative du nombre de décès par rapport au modèle de référence.

•	Verified Tesla Autopilot Death (1 et 2) : Les deux sont très significatifs (p < 0,003).

-Le code 1 (Autopilot engagé) montre curieusement un coefficient négatif (-0,27), suggérant moins de décès par accident que lorsque l'info est inconnue.

-Le code 2 montre un coefficient positif fort (+0,70), indiquant une hausse des décès dans ces circonstances spécifiques. 
...
::::
     ...
     3. La qualité du modèle (𝑅2)

•	Multiple R2 : 0.1565. Cela signifie que le modèle explique environ 15,6 % de la variation du nombre de décès. C'est un score relativement faible (proche de 0), ce qui indique que beaucoup d'autres facteurs non inclus dans le code (vitesse, météo, type de route) influencent grandement la mortalité. 

     4. Les codes de significativité (Les étoiles) 

•	** ou *** : Impact très fiable.

•	* : Impact fiable.

•	. ou rien : Pas d'impact prouvé statistiquement.

En résumé: 

L'analyse par régression montre que l'année et le modèle de véhicule (notamment le Cybertruck) influencent significativement le nombre de décès. Bien que le modèle soit statistiquement valide (𝑝<0,05), il n'explique que 15,6 % de la variance totale, suggérant que d'autres facteurs externes prédominent dans la gravité des accidents." 

Attention : Notez que 393 observations ont été supprimées car des données étaient manquantes (393 observations effacées). Cela peut biaiser les résultats si les données manquantes ne sont pas aléatoires.

##  Frontières et collisions : Analyse mondiale et locale des accidents.
...
#Trier pays par décès décroissant
...
knitr::kable(head(Deaths_per_Country, 20), caption = "Top 20 des décès par pays")
...
#Filtrer données pour États-Unis seulement
...
## Tesla Deaths : Le verdict des données entre IA et erreur humaine

L'analyse des données de mortalité Tesla en 2025 révèle un verdict nuancé : le risque ne disparaît pas avec l'IA, il se transforme. 

L'erreur humaine est souvent la cause initiale (distraction).

L'erreur robotique est l'incapacité du système à compenser cette distraction ou à gérer des situations complexes.

En somme, l'IA Tesla réduit certains types d'accidents (collisions arrières simples) mais en crée de nouveaux, plus complexes et souvent plus médiatisés, liés à la transition critique entre le contrôle humain et l'assistance logicielle. Pour approfondir ces statistiques, vous pouvez consulter les derniers rapports sur TeslaDeaths.com.
...
ggplot(data = Deaths_per_year, aes(x = Year, y = Total_deaths)) +
  ...
  labs(title = "Nombre de décès par an",
              x = "Années",
              y = "Nombre total de décès") +
  theme_minimal()
...
[^1]: Données extraites de la section "Deaths" du site [TeslaDeaths.com](https://www.tesladeaths.com/). Elle comprends 611 données et 21 varriables. Cette base de données collaborative compile les accidents mortels documentés à partir de rapports de police officiels, de coupures de presse et de données de la NHTSA (National Highway Traffic Safety Administration). Elle inclut des variables clés telles que le modèle du véhicule, le rôle des victimes et la confirmation de l'activation du système Autopilot lors de l'impact.
