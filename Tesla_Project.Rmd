...
::: {style="flex: 40%; padding-right: 20px;"}
...
# Vérification de l'existence de coefficients avant de tester
if (length(coef(fit_arima)) > 0) {
  # Si des coefficients existent, on fait le test
  arima_matrix <- lmtest::coeftest(fit_arima)
  arima_df <- as.data.frame(arima_matrix[, , drop = FALSE])
  knitr::kable(arima_df, caption = "Significativité des coefficients ARIMA")
} else {
  # Si aucun coefficient (modèle simple/bruit blanc)
  print("Le modèle ARIMA sélectionné est un modèle vide (bruit blanc ou marche aléatoire). Aucun coefficient à tester.")
}
